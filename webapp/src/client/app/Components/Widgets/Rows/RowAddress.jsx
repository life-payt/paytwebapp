...
import BigWidget from '../BigWidget.jsx';
...
class RowAddress extends React.Component {
  constructor(props){
      super(props);
      this.state = { 
    		party: getParty(), 
		...
		window.payt_session.call('payt.'+window.county+'.db.private.get_producer_real_bill', {args: [this.state.producer_id], callback: function (res){
			if(res != null)
				this.setReal(res.value + this.state.strings.RowAddress.unit);
			else
      ...
      exchange: window.county
		...
		window.payt_session.call('payt.'+window.county+'.db.private.get_producer_simulated_bill', {args: [this.state.producer_id], callback: function (res){
			if(res != null)
				this.setSimul(res.value + this.state.strings.RowAddress.unit);
			else
      ...
      return(
          <div id='row_address'>
    	        <div className="col-lg-3 col-xs-6">
    	          	<BigWidget enable={false} color="bg-green" index={this.state.index}/>
    	        </div>
    	        <div className="col-lg-3 col-xs-6">
    	          <div className="row">
    	            <div id="widget1" className="col-lg-12 col-xs-12">
    	            	<Widget index={this.state.index} field='real' enable={true} value={ this.state.real } text={this.state.strings.RowAddress.realTitle} icon="" color="bg-green disabled" />
    	            	<Spinner config={opts} stopped={this.state.stoppedWidget1}/>
    	            </div>
    	          </div>
    	          <div className="row">
    	            <div id="widget2" className="col-lg-12 col-xs-12">
    	            	<Widget index={this.state.index} field='simulated' enable={true} value={ this.state.simul } text={this.state.strings.RowAddress.simulTitle} icon="" color="bg-green disabled" />
    	            	<Spinner config={opts} stopped={this.state.stoppedWidget2}/>
    	            </div>
    	          </div>
    	        </div>
    	        <div id='knob_widget' className="col-lg-6 col-xs-12">
									<ContainerKnobWidget type='citizen' enable={false} value1={ this.state.selfAvg.toFixed(0) } value2={ this.state.regAvg.toFixed(0) } label1={this.state.strings.RowAddress.labelKnob} month={month}/>
